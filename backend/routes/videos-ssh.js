...
router.get('/folders/:folderId/usage', authMiddleware, async (req, res) => {
  try {
    ...
    const [folderRows] = await db.execute(
      'SELECT identificacao, espaco, espaco_usado FROM streamings WHERE codigo = ? AND codigo_cliente = ?',
      [folderId, userId]
    );
    ...
    const used = folder.espaco_usado || 0;
    const total = folder.espaco || 1000;
    ...
    res.json({
      ...
      usage: {
        ...
        available: available,
        database_used: used,
        real_used: used // Poderia calcular o uso real via SSH se necessário
      }
    });
  } catch (error) {
