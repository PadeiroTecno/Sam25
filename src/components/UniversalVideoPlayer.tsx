...
const UniversalVideoPlayer: React.FC<UniversalVideoPlayerProps> = ({
  ...
  useEffect(() => {
    ...
    // Otimizar URL para vídeos SSH
    let videoUrl = buildVideoUrl(src);
    if (src.includes('/api/videos-ssh/')) {
      videoUrl = optimizeSSHVideoUrl(src);
    }
    ...
    console.log('🎥 Configurando vídeo:', {
      ...
      optimized: videoUrl !== buildVideoUrl(src)
    });
          ...
          if (data.fatal) {
            ...
            if (src && src.includes('/api/videos-ssh/')) {
              if (data.details?.includes('404')) {
                ...
                errorMsg = 'Erro ao acessar vídeo. Tentando método alternativo...';
                // Tentar fallback para URL direta
                setTimeout(() => {
                  const fallbackUrl = buildVideoUrl(src);
                  if (fallbackUrl !== videoUrl) {
